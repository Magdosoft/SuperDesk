...
mydata={
    "test_reuter": {"username": "Ahram.API","password": "yQeTRqsEaSalXlK"},
    "provider2": {"username": "user2","password": "pass2"},
    "provider3": {"username": "user3","password": "pass3"},
    "provider4": {"username": "user4","password": "pass4"},
    "provider5": {"username": "user5","password": "pass5"}
...
class HTTPFeedingService(FeedingService, metaclass=ABCMeta):
    ...
    def _generate_auth_token(self, provider):
        ...
        payload = {
            'username': mydata[provider['name']]['username'],
            'password': mydata[provider['name']]['password'],
        }

	#logger.warn(provider['name'])
	
	#payload = {
        #    'username': 'Ahram.API',
        #    'password': 'yQeTRqsEaSalXlK',
        #}
