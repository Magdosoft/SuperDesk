...
class DpaNewsMLTwoFeedParser(XMLFeedParser):
    """
    ...
    def parse_inline_content(self, tree, item, ns=NS['xhtml']):
        if tree.get('contenttype') == NITF:
            ...
            section = body.find(self.qname('section', ns))
            elements = []
            for elem in section:
                if elem.text:
            ...
            if len(elements) == 1 and get_content_tag(section[0]) == 'p':
            ...
            if len(elements) > 0:
                ...
                content['content'] = '<pre>' + section.text + '</pre>'
...
register_feed_parser(DpaNewsMLTwoFeedParser.NAME, DpaNewsMLTwoFeedParser())
